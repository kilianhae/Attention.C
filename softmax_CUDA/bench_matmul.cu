...
void run_sgemm_cublas_batched(float* A, float* B, float* C, int M, int N, int K, bool transpose){
    ...
    if(transpose){
      ...
      stat = cublasSgemmBatched(handle, CUBLAS_OP_T, CUBLAS_OP_N, K, N, M, &alpha, Barray_d, M , Aarray_d, K, &beta, Carray_d, K, BB*H);
      cudaDeviceSynchronize();
      end = getTimeStamp();
...
int main(){
    ...
    run_sgemm_cublas_batched(d_A,d_B,d_C,M,N,K,true);
    //run_sgemm_cublas(d_A,d_B,d_C,M,N,K,false);
